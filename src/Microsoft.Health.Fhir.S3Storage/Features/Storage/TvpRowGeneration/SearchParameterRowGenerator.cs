...
{
    internal abstract class SearchParameterRowGenerator<TSearchValue, TRow> : ITableValuedParameterRowGenerator<S3ResourceMetadata, TRow>
        where TRow : struct
    {
        private readonly bool _isConvertSearchValueOverridden;
        private bool _isInitialized;

        protected SearchParameterRowGenerator(S3StorageFhirModel model)
        {
        ...
        protected S3StorageFhirModel Model { get; }

        public virtual IEnumerable<TRow> GenerateRows(S3ResourceMetadata input)
        {
            EnsureInitialized();
