...
(
    ...
    LinkToRawResource varchar(2048) NULL,
    IsS3ObjectValid bit NOT NULL
...
AS
    ...
    IF (@etag IS NOT NULL AND @etag <> @previousVersion) BEGIN
        Throw 150412 , 'Precondition failed', 1;
    END
    ...
    INSERT INTO dbo.Resource
        (ResourceTypeId, ResourceId, Version, IsHistory, ResourceSurrogateId, IsDeleted, RequestMethod, RawResource, IsS3ObjectValid)
    VALUES
        (@resourceTypeId, @resourceId, @version, 0, @resourceSurrogateId, @isDeleted, @requestMethod, @rawResource, @isS3ObjectValid)
...
/*************************************************************
    Stored procedures for creating and deleting with S3 Link
**************************************************************/
...
--         The version of the resource as a result set. Will be empty if no insertion was done.
--
CREATE PROCEDURE dbo.UpsertResourceLink
    @baseResourceSurrogateId bigint,
    @resourceTypeId smallint,
    @resourceId varchar(64),
    @eTag int = NULL,
    ...
    @keepHistory bit,
    @requestMethod varchar(10),
    @linkToRawResource varchar(2048),
    @isS3ObjectValid bit,
    @resourceWriteClaims dbo.ResourceWriteClaimTableType_1 READONLY,
    @compartmentAssignments dbo.CompartmentAssignmentTableType_1 READONLY,
    @referenceSearchParams dbo.ReferenceSearchParamTableType_1 READONLY,
...
AS
    ...
    INSERT INTO dbo.Resource
        (ResourceTypeId, ResourceId, Version, IsHistory, ResourceSurrogateId, IsDeleted, RequestMethod, LinkToRawResource, IsS3ObjectValid)
    VALUES
        (@resourceTypeId, @resourceId, @version, 0, @resourceSurrogateId, @isDeleted, @requestMethod,  @linkToRawResource, @isS3ObjectValid)
...
AS
    ...
    IF (@version IS NULL) BEGIN
        SELECT ResourceSurrogateId, Version, IsDeleted, IsHistory, RawResource, IsS3ObjectValid, LinkToRawResource
        FROM dbo.Resource
        WHERE ResourceTypeId = @resourceTypeId AND ResourceId = @resourceId AND IsHistory = 0
    END
    ELSE BEGIN
...
--         A result set with 0 or 1 rows.
--
CREATE PROCEDURE dbo.ReadResourceLink
    @resourceTypeId smallint,
    @resourceId varchar(64),
    @version int = NULL
AS
    ...
    IF (@version IS NULL) BEGIN
        SELECT ResourceSurrogateId, Version, IsDeleted, IsHistory, IsS3ObjectValid, LinkToRawResource
        FROM dbo.Resource
        WHERE ResourceTypeId = @resourceTypeId AND ResourceId = @resourceId AND IsHistory = 0
    END
    ELSE BEGIN
