...
{
    /// <summary>
    /// Combines <see cref="S3TableExpression"/>s that are over the same search parameter.
    ...
    /// </summary>
    internal class TableExpressionCombiner : S3SqlExpressionRewriterWithInitialContext<object>
    {
        ...
        public override Expression VisitSqlRoot(S3SqlRootExpression expression, object context)
        {
            ...
            var newTableExpressions = expression.TableExpressions
                ...
                .SelectMany(g =>
                {
                    ...
                    {
                        return (IEnumerable<S3TableExpression>)g;
                    }

                    var childExpressions = new List<Expression>();
                    foreach (S3TableExpression tableExpression in g)
                    {
                        Debug.Assert(tableExpression.Kind == S3TableExpressionKind.Normal, "Kind should not be set yet");
                    ...
                    return new[] { new S3TableExpression(g.Key.queryGenerator, Expression.SearchParameter(g.Key.searchParameter, Expression.And(childExpressions)), null, S3TableExpressionKind.Normal) };
                }).ToList();
